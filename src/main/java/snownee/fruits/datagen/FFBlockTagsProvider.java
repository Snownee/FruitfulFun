...
import static net.minecraft.tags.BlockTags.FLOWER_POTS;
...
//		helper.getEntriesByModule("core", "cherry", "food").forEach(this::processTools);
		...
		getOrCreateTagBuilder(CITRUS_LOGS).add(CITRUS_LOG.get(), CITRUS_WOOD.get(), STRIPPED_CITRUS_LOG.get(), STRIPPED_CITRUS_WOOD.get());
		...
		tag(LOGS_THAT_BURN).addTag(CITRUS_LOGS).addTag(REDLOVE_LOGS);
		...
		getOrCreateTagBuilder(BlockTags.WOODEN_SLABS).add(CITRUS_STAIRS.get()).addOptional(REDLOVE_STAIRS.key());
		...
		getOrCreateTagBuilder(FLOWER_POTS).add(POTTED_CITRON.get(), POTTED_GRAPEFRUIT.get(), POTTED_LEMON.get(), POTTED_LIME.get(), POTTED_TANGERINE.get(), POTTED_ORANGE.get(), POTTED_POMELO.get())
