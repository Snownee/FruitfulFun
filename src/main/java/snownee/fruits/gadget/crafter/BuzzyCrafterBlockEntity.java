...
import snownee.fruits.gadget.scent.ScentedCandleItem;
...
public class BuzzyCrafterBlockEntity extends BeehiveBlockEntity implements BuzzyCrafterContainer {
	...
	private void maybePopItem() {
		...
		Hooks.popItemBelow(this, true, 0);
	}

	public boolean isBlocked() {
		if (blocked == TriState.DEFAULT) {
			Objects.requireNonNull(level);
			blocked = TriState.from(BuzzyCrafterBlock.blocksContainer(level.getBlockState(worldPosition.above())));
		}
