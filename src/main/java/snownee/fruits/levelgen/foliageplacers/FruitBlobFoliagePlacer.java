...
public class FruitBlobFoliagePlacer extends BlobFoliagePlacer {
	...
	protected void createFoliage(LevelSimulatedReader pLevel, FoliageSetter foliageSetter, RandomSource pRandom, TreeConfiguration pConfig, int pMaxFreeTreeHeight, FoliageAttachment pAttachment, int pFoliageHeight, int pFoliageRadius, int pOffset) {
		for (int i = pOffset; i >= pOffset - pFoliageHeight; --i) {
			...
			placeLeavesRow(pLevel, foliageSetter, pRandom, pConfig, pAttachment.pos(), j, i, pAttachment.doubleTrunk());
	...
	@Override
	protected void placeLeavesRow(LevelSimulatedReader p_161438_, FoliageSetter foliageSetter, RandomSource p_161440_, TreeConfiguration p_161441_, BlockPos p_161442_, int p_161443_, int p_161444_, boolean p_161445_) {
		int i = p_161445_ ? 1 : 0;
		...
		for (int j = -p_161443_; j <= p_161443_ + i; ++j) {
			for (int k = -p_161443_; k <= p_161443_ + i; ++k) {
				if (!shouldSkipLocationSigned(p_161440_, j, p_161444_, k, p_161443_, p_161445_)) {
					...
					tryPlaceLeaf(p_161438_, foliageSetter, p_161440_, p_161441_, blockpos$mutableblockpos);
				}
			}
		}
	}
